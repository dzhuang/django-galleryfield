...
from tests.utils import get_upload_file_path, remove_upload_directory, test_media_root
...
class ViewTestMixin(UserCreateMixin):
    ...
    def _get_crop_post_data(
        cropped_result=None, thumbnail_size=defaults.DEFAULT_THUMBNAIL_SIZE
    ):
        data = {}
        if cropped_result:
    ...
    def demo_upload_file(self, user, file_obj, force_login=True, xml_request=True):
        ...
        with open(file_obj, "rb") as fp:
            ...
            resp = self.c.post(
                self.get_demo_upload_url(), data={"files[]": fp}, **request_kwargs
            )
        return resp
...
class GalleryWidgetUploadViewTest(ViewTestMixin, TestCase):
    ...
    def test_upload_fail_not_xml_request(self):
        resp = self.demo_upload_file(
            self.user, find("demo/screen_upload.png"), xml_request=False
        )
    ...
    def test_upload_not_logged_in(self):
        resp = self.demo_upload_file(
            self.user, find("demo/screen_upload.png"), force_login=False
        )
    ...
    def test_cbv_target_model_not_configured(self):
        ...
        with self.assertRaises(ImproperlyConfigured) as cm:
            built_in_views.BuiltInImageCreateView.as_view(target_model=None)(request)

        expected_msg = (
            "Using BaseImageModelMixin (base class of "
    ...
    def test_cbv_invalid_crop_url(self):
        ...
        with self.assertRaises(ImproperlyConfigured) as cm:
            built_in_views.BuiltInImageCreateView.as_view(
                crop_url_name="invalid-crop-url"
            )(request)

        expected_msg = (
            "'crop_url_name' in BuiltInImageCreateView "
    ...
    def test_cbv_crop_url_not_configured(self):
        ...
        resp = built_in_views.BuiltInImageCreateView.as_view(crop_url_name=None)(
            request
        )
...
class GalleryWidgetFetchViewTest(ViewTestMixin, TestCase):
    def test_fetch(self):
        ...
        factories.DemoGalleryFactory.create(
            creator=self.user, number_of_images=2, shuffle=True
        )

        def get_fetched_result():
            resp = self.c.get(
                self.get_demo_fetch_url(params={"pks": list(gallery.images)}),
                HTTP_X_REQUESTED_WITH="XMLHttpRequest",
            )
            self.assertEqual(resp.status_code, 200)
            resp_image_list = [file["pk"] for file in json.loads(resp.content)["files"]]
    ...
    def test_fetch_thumbnail_size_list(self):
        ...
        url = self.get_demo_fetch_url(params={"pks": list(gallery.images)})
    ...
    def test_fetch_without_pks(self):
        ...
        self.c.force_login(self.user)
        resp = self.c.get(
            self.get_demo_fetch_url(params={"not-pks": list(gallery.images)}),
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        self.assertEqual(resp.status_code, 400)
    ...
    def test_fetch_pk_not_digit(self):
        self.c.force_login(self.user)
        resp = self.c.get(
            self.get_demo_fetch_url(params={"pks": [1.1]}),
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        self.assertEqual(resp.status_code, 400)
    ...
    def test_fetch_pk_decode_error(self):
        ...
        self.c.force_login(self.user)
        resp = self.c.get(
            self.get_demo_fetch_url(params={"pks": "abcd"}),
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        self.assertEqual(resp.status_code, 400)
    ...
    def test_fetch_pk_loaded_but_not_list(self):
        ...
        self.c.force_login(self.user)
        resp = self.c.get(
            self.get_demo_fetch_url(params={"pks": '{"ab": "cd"}'}),
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        self.assertEqual(resp.status_code, 400)
    ...
    def test_cbv_target_model_not_configured(self):
        ...
        with self.assertRaises(ImproperlyConfigured) as cm:
            built_in_views.BuiltInImageListView.as_view(target_model=None)(request)
    ...
    def test_cbv_invalid_crop_url(self):
        ...
        expected_msg = (
            "'crop_url_name' in BuiltInImageListView "
            "is invalid. The exception is: NoReverseMatch: "
            "Reverse for 'invalid-crop-url' not found."
        )
    ...
    def test_cbv_crop_url_not_configured(self):
        ...
        resp = built_in_views.BuiltInImageListView.as_view(crop_url_name=None)(request)
    ...
    def test_cbv_thumbnail_size_invalid(self):
        ...
        request = self.factory.get(
            self.get_demo_fetch_url(
                params={"pks": list(gallery.images), "thumbnail_size": 100.1}
            ),
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
        )
        request.user = self.user
    ...
    def test_cbv_image_unexpectedly_deleted(self):
        ...
        for f in resp_files:
            self.assertEqual(
                f["error"], "image: The image was unexpectedly deleted from server"
            )
    ...
    def test_cbv_get_thumbnail_error(self):
        ...
        resp_files = json.loads(resp.content)["files"]
        for f in resp_files:
            self.assertEqual(f["error"], "thumbnail: RuntimeError: Unexpected error")
...
class GalleryWidgetCropViewTest(ViewTestMixin, TestCase):
    ...
    def test_crop_png(self):
        ...
        self.c.force_login(self.user)
        resp = self.c.post(
            self.get_demo_crop_url(1), data=data, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
        self.assertEqual(resp.status_code, 200)
    ...
    def test_cbv_target_model_not_configured(self):
        ...
        with self.assertRaises(ImproperlyConfigured) as cm:
            built_in_views.BuiltInImageCropView.as_view(target_model=None)(
                request, **kwargs
            )
    ...
    def test_cbv_invalid_crop_url(self):
        ...
        expected_msg = (
            "'crop_url_name' in BuiltInImageCropView "
            "is invalid. The exception is: NoReverseMatch: "
            "Reverse for 'invalid-crop-url' not found."
        )
    ...
    def test_cbv_crop_url_not_configured(self):
        ...
        resp = built_in_views.BuiltInImageCropView.as_view(crop_url_name=None)(
            request, **kwargs
        )
