...
from galleryfield.utils import (
    convert_dict_to_plain_text,
...
class GalleryWidget(forms.HiddenInput):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.fetch_url = None if disable_fetch else fetch_url
    ...
    def jquery_file_upload_ui_options(self):
        return (
            self._jquery_file_upload_ui_options
    ...
    def jquery_file_upload_ui_options(self, options):
        ...
        ju_settings = conf.JQUERY_FILE_UPLOAD_UI_DEFAULT_OPTIONS.copy()
        ...
        if (
            "singleFileUploads" in ju_settings
            ...
            logger.warning(
                ...
                "ignored." % {"obj": self.__class__.__name__}
            )
        ...
        if "previewMaxWidth" in ju_settings or "previewMaxHeight" in ju_settings:
            logger.warning(
                ...
                "thumbnail_size='120x60'" % {"obj": self.__class__.__name__}
            )
    ...
    def set_and_check_urls(self):
        ...
        try:
            ...
            raise ImproperlyConfigured(
                ...
                "a valid URL nor a valid URL name." % self.upload_url
            )
        ...
        if self.disable_fetch:
            ...
            try:
                ...
                raise ImproperlyConfigured(
                    ...
                    "a valid URL nor a valid URL name." % self.fetch_url
                )
        ...
        conflict_config = []
        upload_url_is_default = self.upload_url == get_url_from_str(
            defaults.DEFAULT_UPLOAD_URL_NAME, require_urlconf_ready=True
        )

        if upload_url_is_default:
            conflict_config.append({"param": "upload_url", "value": self.upload_url})

        if self.fetch_url:
            fetch_url_is_default = self.fetch_url == get_url_from_str(
                defaults.DEFAULT_FETCH_URL_NAME, require_urlconf_ready=True
            )
            if fetch_url_is_default:
                conflict_config.append({"param": "fetch_url", "value": self.fetch_url})
    ...
    def get_stringfied_jquery_file_upload_ui_options(self):
        ...
        max_number_of_images = getattr(self, "max_number_of_images", None)
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        context = {
            ...
            "prompt_alert_on_window_reload_if_changed": conf.PROMPT_ALERT_ON_WINDOW_RELOAD_IF_CHANGED,
        }
        ...
        if _context["widget"]["attrs"].get("disabled", False) or _context["widget"][
            "attrs"
        ].get("readonly"):
